...
export async function searchGoogleResults(query, options = {}) {
  ...
  try {
    ...
    if (status === 429 || status === 403 || providerMessage.toLowerCase().includes('quota') || providerMessage.toLowerCase().includes('limit')) {
      reportSystemError('SERP_API_LIMIT', wrappedError, { query, options });
    }
