...
function getClientPromise() {
  if (!cache.promise) {
    if (!client) {
      // Removing aggressive 5000ms timeouts. Using default 30s serverSelectionTimeoutMS
      // allows the driver to gracefully survive replica set elections without throwing.
      // maxIdleTimeMS: 10000 prevents stale sockets in serverless/container NAT environments.
    ...
    cache.promise = client.connect().catch(err => {
      ...
      cache.promise = null
      client = null // Ensure client is also recreated on retry
      throw err
    })
  }
