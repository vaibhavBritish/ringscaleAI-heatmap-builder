...
export async function generateAndStoreAuditReport(projectId) {
  ...
  const auditReport = {
    ...
    keywords: {
      activeCampaign: [], // will be populated below
      aiSuggested: [
  ...
  // 5.5 Fetch active keywords from GMB Connector Database
  try {
    const gmbApiUrl = process.env.NEXT_PUBLIC_BASE_URL?.includes('connect.ringscale.ai') 
      ? `https://connect.ringscale.ai/api/keywords?businessName=${encodeURIComponent(googleData.name || project.businessName)}`
      : (process.env.GMB_API_URL ? `${process.env.GMB_API_URL}?businessName=${encodeURIComponent(googleData.name || project.businessName)}` : null);
    ...
    if (gmbApiUrl) {
      ...
      if (res.ok) {
        ...
        if (businesses.length > 0 && businesses[0].keywords) {
          auditReport.keywords.activeCampaign = businesses[0].keywords;
        }
      }
    } else {
      ...
      const businessNameQuery = googleData.name || project.businessName
      ...
      for (const business of businesses) {
        ...
        // Fetch locations for this business, trying both ObjectId and String forms
      ...
      if (allKeywords.size > 0) {
        auditReport.keywords.activeCampaign = Array.from(allKeywords)
      }
      
      await client.close()
    }
  } catch (err) {
    console.error('Failed to fetch GMB keywords:', err)
  }
