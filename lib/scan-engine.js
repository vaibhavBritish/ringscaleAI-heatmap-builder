    ...
    try {
      for (const point of scanPoints) {
        ...
        try {
          ...
          console.error(`Error processing point ${point.id} in job ${scanJobId}:`, pointError)
          // Don't stop the whole scan for one point error, just record it
          ...
          results.push({ error: pointError.message })
        }
